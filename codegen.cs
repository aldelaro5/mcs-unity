...
namespace Mono.CSharp
{
	...
	public class EmitContext : BuilderContext
	{
        ...
        private readonly FieldInfo _ilGenratorCodeLen = typeof(ILGenerator).GetField("code_len", BindingFlags.Instance | BindingFlags.NonPublic);
...
#if NET35
            methodSymbols.MarkSequencePoint ((int)_ilGenratorCodeLen.GetValue(ig), sf.SourceFileEntry, loc.Row, loc.Column, false);
#else
...
#if NET35
            methodSymbols.StartBlock (CodeBlockEntry.Type.Lexical, (int)_ilGenratorCodeLen.GetValue(ig), scopeIndex);
#else
...
#if NET35
            methodSymbols.StartBlock (CodeBlockEntry.Type.CompilerGenerated, (int)_ilGenratorCodeLen.GetValue(ig), scopeIndex);
#else
		...
		public void EndScope ()
		{
			if ((flags & Options.OmitDebugInfo) != 0)
				return;
#if NET35
            methodSymbols.EndBlock ((int)_ilGenratorCodeLen.GetValue(ig));
#else
